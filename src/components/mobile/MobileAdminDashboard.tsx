...
import { Users, Building2, FileText, CheckCircle, Loader, Database } from 'lucide-react';
...
export default function MobileAdminDashboard() {
  ...
  async function loadData() {
    try {
      ...
      const engineersData = StorageService.getEngineers();
            ...
            {reports.slice(0, 5).map((report) => {
              ...
              return (
                <div key={report.id} className="border-b border-slate-100 pb-2 last:border-0">
                  <p className="text-sm font-medium text-slate-900">{engineer?.name}</p>
                  <p className="text-xs text-slate-600 mt-1">{client?.name}</p>
                </div>
              );
            })}
          </div>
...
function ClientsTab({ clients }: { clients: Client[] }) {
  return (
    <div className="space-y-3">
      {clients.map((client) => (
        <div key={client.id} className="bg-white rounded-lg shadow-sm border border-slate-200 p-4">
          ...
          {client.company && (
            <p className="text-xs text-slate-500 mt-2">{client.company}</p>
          )}
        </div>
      ))}
      {clients.length === 0 && (
